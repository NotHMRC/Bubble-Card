...
| `--bubble-sub-button-outline` | `box-shadow` | Outline added to a slider sub-button, only when it paints the same color as the card behind it, which would leave nothing on screen to show that a slider is there (set it to `none` to remove it) |
...
| `--bubble-sub-slider-outline` | `box-shadow` | Same outline, kept for clarity, falls back to `--bubble-sub-button-outline` |
