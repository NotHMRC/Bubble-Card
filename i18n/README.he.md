...
**[`התקנה`](#התקנה)**  **[`תצורה`](#תצורה)**  **[`חלון קופץ`](#חלון-קופץ)**  **[`ערימת כפתורים אופקית`](#ערימת-כפתורים-אופקית)**  **[`כפתור`](#כפתור)**  **[`נגן מדיה`](#נגן-מדיה)**  **[`תריס`](#תריס)**  **[`בחירה`](#בחירה)**  **[`מיזוג אקלים`](#מיזוג-אקלים)**  **[`לוח שנה`](#לוח-שנה)**  **[`מפריד`](#מפריד)**  **[`עמודה ריקה`](#עמודה-ריקה)**  **[`כפתורי משנה בלבד`](#כפתורי-משנה-בלבד)**  **[`כפתורי משנה`](#כפתורי-משנה)**  **[`הצעות ישויות`](#הצעות-ישויות)**  **[`פריסות הכרטיס`](#פריסות-הכרטיס)**  **[`תנאים`](#תנאים)**  **[`פעולות`](#פעולות-הקשה-הקשה-כפולה-ולחיצה-ארוכה)**  **[`עיצוב`](#עיצוב)**  **[`תבניות`](#תבניות)**  **[`מודולים`](#מודולים)**  **[`לוקליזציה`](#לוקליזציה)**  **[`עזרה`](#עזרה)**  **[`תרומה לפרויקט`](#תרומה-לפרויקט)**  **[`תרומה כספית`](#תרומה-כספית)**
...
הוא עובד גם עם כל ישות שחושפת את האפשרויות שלה כרשימת מאפיינים: `hvac_modes`, `fan_modes`, `swing_modes`, `swing_horizontal_modes` ו-`preset_modes` במיזוג אקלים, `available_modes` במכשיר אדים, `operation_list` במחמם מים, `effect_list` בתאורה, `source_list` ו-`sound_mode_list` בנגן מדיה.
...
הכרטיס הזה מאפשר לך לשלוט בישויות `climate`, `humidifier` ו-`water_heater` שלך. מכשיר אדים, מייבש אוויר או היגרוסטט כללי מקבל את אותן בקרות פלוס ומינוס עבור לחות היעד שלו, ומחמם מים עבור טמפרטורת היעד שלו.

> [!TIP]
> תפריט בחירת המצב הוא [כפתור משנה](#כפתורי-משנה) שמתווסף אוטומטית ביצירת הכרטיס. ניתן לשנות או להסיר אותו כרצונך. הוא קורא את `hvac_modes` של ישות מיזוג אקלים, את `available_modes` של מכשיר אדים ואת `operation_list` של מחמם מים.
...
| שם                     | סוג    | דרישה                         | אפשרויות נתמכות                                  | תיאור                                                                                                     |
|--------------------------|---------|-------------------------------------|--------------------------------------------------|-----------------------------------------------------------------------------------------------------------------|
| `entity`                | string  | **נדרש**                        | ישות מיזוג אקלים, מכשיר אדים או מחמם מים           | הישות לשליטה (למשל, `climate.living_room`, `humidifier.bedroom` או `water_heater.boiler`).             |
...
| `state_content`         | string or list | אופציונלי | `state`, שם מאפיין, תבנית | מה מציגה השורה שמתחת לשם, ראה את אפשרויות הכפתור. המפתח הישן `show_state` עדיין עובד. |
...
| `state_color`           | boolean | אופציונלי                            | `true` או `false` (ברירת מחדל)                     | מחיל צבע רקע קבוע כאשר הישות דולקת. למחמם מים אין מקבילה ל-`hvac_action`, ולכן זה הדבר היחיד שצובע אותו.     |
| `step` | number | אופציונלי | כל מספר | הצעד של ערך היעד, טמפרטורה או לחות. |
| `min_temp` | number | אופציונלי | כל מספר | ערך היעד המינימלי. במכשיר אדים מדובר בלחות ולא בטמפרטורה. |
| `max_temp` | number | אופציונלי | כל מספר | ערך היעד המקסימלי. במכשיר אדים מדובר בלחות ולא בטמפרטורה. |
| `button_action` | object | אופציונלי | `tap_action`, `double_tap_action` או `hold_action`, ראה [פעולות](#פעולות-הקשה-הקשה-כפולה-ולחיצה-ארוכה) | מאפשר לשנות את הפעולות המובנות בברירת מחדל בלחיצה על הכפתור. |
| `tap_action` | object | אופציונלי | ראה [פעולות](#פעולות-הקשה-הקשה-כפולה-ולחיצה-ארוכה) | הגדר את סוג הפעולה בלחיצה על האייקון, אם לא מוגדר, ייעשה שימוש ב-`more-info`. |
| `double_tap_action` | object | אופציונלי | ראה [פעולות](#פעולות-הקשה-הקשה-כפולה-ולחיצה-ארוכה) | הגדר את סוג הפעולה בהקשה כפולה על האייקון, אם לא מוגדר, ייעשה שימוש ב-`none`. |
| `hold_action` | object | אופציונלי | ראה [פעולות](#פעולות-הקשה-הקשה-כפולה-ולחיצה-ארוכה) | הגדר את סוג הפעולה בלחיצה ארוכה על האייקון, אם לא מוגדר, ייעשה שימוש ב-`more-info`. |                              |
...
| `--bubble-state-humidifier-on-color` | `color` | צבע שכבת-על עבור מכשיר אדים שפועל |
| `--bubble-state-humidifier-humidifier-on-color` | `color` | צבע שכבת-על עבור מכשיר אדים פועל, כאשר מחלקת ההתקן שלו היא `humidifier` |
| `--bubble-state-humidifier-dehumidifier-on-color` | `color` | צבע שכבת-על עבור מייבש אוויר פועל, כאשר מחלקת ההתקן שלו היא `dehumidifier` |
| `--bubble-state-water_heater-<operation>-color` | `color` | צבע שכבת-על עבור מצב הפעלה של מחמם מים, למשל `--bubble-state-water_heater-eco-color` |
