...
**[`Установка`](#установка)**  **[`Конфигурация`](#конфигурация)**  **[`Pop-up`](#pop-up)**  **[`Горизонтальный стек кнопок`](#горизонтальный-стек-кнопок)**  **[`Кнопка`](#кнопка)**  **[`Медиаплеер`](#медиаплеер)**  **[`Шторы`](#шторы)**  **[`Выбор`](#выбор)**  **[`Климат`](#климат)**  **[`Календарь`](#календарь)**  **[`Разделитель`](#разделитель)**  **[`Пустой столбец`](#пустой-столбец)**  **[`Только дополнительные кнопки`](#только-дополнительные-кнопки)**  **[`Дополнительные кнопки`](#дополнительные-кнопки)**  **[`Предложения для сущностей`](#предложения-для-сущностей)**  **[`Компоновки карточки`](#компоновки-карточки)**  **[`Условия`](#условия)**  **[`Действия`](#действия-при-нажатии-двойном-нажатии-и-удержании)**  **[`Оформление`](#оформление)**  **[`Шаблоны`](#шаблоны)**  **[`Modules`](#modules)**  **[`Локализация`](#локализация)**  **[`Помощь`](#помощь)**  **[`Участие в проекте`](#участие-в-проекте)**  **[`Поддержать проект`](#поддержать-проект)**
...
| `popup_style` | string | Опционально | `bubble` (по умолчанию), `classic` или `home-assistant` | Определяет визуальный стиль всплывающего окна |
...
| `state_content` | string or list | Опционально | `state`, `last-changed`, `last-updated`, `last-triggered`, имя атрибута, например `brightness` или `forecast[0].temperature`, или [шаблон](#шаблоны) | Что показывает строка под именем, в этом порядке. Без этой настройки кнопка `button_type: state` показывает то, что Home Assistant показывает для сущности (её состояние, а также текущую температуру сущности климата, положение шторы, яркость света). Старые ключи `show_state`, `show_attribute`, `attribute`, `show_last_changed` и `show_last_updated` по-прежнему работают и переписываются в `state_content`, когда вы открываете редактор. |
...
| `state_content` | string or list | Необязательно | `state`, `last-changed`, `last-updated`, `last-triggered`, имя атрибута, например `brightness` или `forecast[0].temperature`, или [шаблон](#шаблоны) | Что показывает строка под именем, в этом порядке. Без этой настройки кнопка `button_type: state` показывает то, что Home Assistant показывает для сущности (её состояние, а также текущую температуру сущности климата, положение шторы, яркость света). Старые ключи `show_state`, `show_attribute`, `attribute`, `show_last_changed` и `show_last_updated` по-прежнему работают и переписываются в `state_content`, когда вы открываете редактор. |
...
Она также работает с любой сущностью, которая предоставляет свои опции в виде списка атрибутов: `hvac_modes`, `fan_modes`, `swing_modes`, `swing_horizontal_modes` и `preset_modes` у сущности климата, `available_modes` у увлажнителя, `operation_list` у водонагревателя, `effect_list` у света, `source_list` и `sound_mode_list` у медиаплеера.
...
Эта карточка позволяет управлять вашими сущностями `climate`, `humidifier` и `water_heater`. Увлажнитель, осушитель или универсальный гигростат получает те же кнопки плюс и минус для своей целевой влажности, а водонагреватель получает их для своей целевой температуры.

> [!TIP]
> Меню выбора режима - это [дополнительная кнопка](#дополнительные-кнопки), которая добавляется автоматически при создании карточки. Затем вы можете изменить или удалить её по своему усмотрению. Оно читает `hvac_modes` сущности климата, `available_modes` увлажнителя и `operation_list` водонагревателя.
...
| Название                     | Тип    | Требование                         | Поддерживаемые опции                                  | Описание                                                                                                     |
|--------------------------|---------|-------------------------------------|--------------------------------------------------|-----------------------------------------------------------------------------------------------------------------|
| `entity`                | string  | **Обязательно**                        | Сущность климата, увлажнителя или водонагревателя  | Сущность для управления (например, `climate.living_room`, `humidifier.bedroom` или `water_heater.boiler`).            |
...
| `state_content`         | string or list | Необязательно | `state`, имя атрибута, шаблон | Что показывает строка под именем, см. настройки кнопки. Старый ключ `show_state` по-прежнему работает. |
...
| `state_color`           | boolean | Необязательно                            | `true` или `false` (по умолчанию)                     | Применяет постоянный цвет фона, когда сущность включена. У водонагревателя нет аналога `hvac_action`, поэтому это единственное, что его окрашивает. |
| `step` | number | Необязательно | Любое число | Шаг изменения целевого значения, температуры или влажности. |
| `min_temp` | number | Необязательно | Любое число | Минимальное целевое значение. У увлажнителя это влажность, а не температура. |
| `max_temp` | number | Необязательно | Любое число | Максимальное целевое значение. У увлажнителя это влажность, а не температура. |
| `button_action` | object | Необязательно | `tap_action`, `double_tap_action` или `hold_action`, см. [действия](#действия-при-нажатии-двойном-нажатии-и-удержании) | Позволяет изменить действия по умолчанию при нажатии на кнопку. |
| `tap_action` | object | Необязательно | См. [действия](#действия-при-нажатии-двойном-нажатии-и-удержании) | Определяет тип действия при нажатии на значок, если не задано, будет использовано `more-info`. |
| `double_tap_action` | object | Необязательно | См. [действия](#действия-при-нажатии-двойном-нажатии-и-удержании) | Определяет тип действия при двойном нажатии на значок, если не задано, будет использовано `none`. |
| `hold_action` | object | Необязательно | См. [действия](#действия-при-нажатии-двойном-нажатии-и-удержании) | Определяет тип действия при удержании значка, если не задано, будет использовано `more-info`. |                              |
...
| `--bubble-state-humidifier-on-color` | `color` | Цвет наложения для работающего увлажнителя |
| `--bubble-state-humidifier-humidifier-on-color` | `color` | Цвет наложения для работающего увлажнителя, когда его класс устройства `humidifier` |
| `--bubble-state-humidifier-dehumidifier-on-color` | `color` | Цвет наложения для работающего осушителя, когда его класс устройства `dehumidifier` |
| `--bubble-state-water_heater-<operation>-color` | `color` | Цвет наложения для режима работы водонагревателя, например `--bubble-state-water_heater-eco-color` |
...
> [!NOTE]
> Условия вычисляются в вашем браузере, поэтому те немногие из них, которым нужен сервер Home Assistant, не могут быть точными: восход и закат читаются из сущности `sun.sun`, а не пересчитываются, а длительность `for` измеряется от последнего изменения состояния, без истории recorder. Исключение составляет `template`, который отрисовывается сервером, как любой другой [шаблон Home Assistant](#шаблоны).
>
...
Bubble Card поддерживает два вида шаблонов:

- **Шаблоны Home Assistant (Jinja)**, те же, что вы уже пишете в автоматизациях, в Mushroom или в card-mod. Поместите `{{ ... }}` или `{% ... %}` в поддерживаемое поле, и Home Assistant отрисует его для вас в реальном времени.
- **JavaScript-шаблоны**, `${ ... }` в ваших [пользовательских стилях](#оформление), для всего, что должно дотянуться до самой карточки.

### Шаблоны Home Assistant (Jinja)

Шаблоны отрисовываются вашим сервером Home Assistant и обновляются сами, когда меняется то, что они читают. Они работают в этих полях:

<details>

<summary><b>Поддерживаемые поля (с примерами)</b></summary>

| Поле | Пример |
| --- | --- |
| `name`, на каждой карточке (включая заголовок всплывающего окна и разделитель) | `name: "{{ states('sensor.living_temp') }} °C"` |
| `icon`, на каждой карточке (а также `icon_open`, `icon_close`, `icon_up` и `icon_down` шторы) | `icon: "{{ 'mdi:window-open' if is_state('binary_sensor.window', 'on') else 'mdi:window-closed' }}"` |
| `name` и `icon` [дополнительной кнопки](#дополнительные-кнопки) | `name: "{{ 'Wet' if states(entity) \| float > 60 else 'Dry' }}"` |
| `state_content`, на карточке или дополнительной кнопке, рядом со `state` и именами атрибутов | `state_content: [state, "{{ states('sensor.humidity') }} %"]` |
| `1_name`, `1_icon`... [горизонтального стека кнопок](#горизонтальный-стек-кнопок) | `1_name: "{{ user }}"` |
| `styles` карточки и код [модуля](#modules), вперемешку с JavaScript-шаблонами | см. ниже |
| [Условия](#условия), с `condition: template` | `value_template: "{{ is_state('sun.sun', 'below_horizon') }}"` |

</details>

> [!IMPORTANT]
> Всегда заключайте шаблон в кавычки. Без них `name: {{ states('x') }}` читается YAML как словарь, а не как текст, и карточка его отклоняет.

Три переменные доступны в дополнение ко всему, что предлагает Home Assistant (`states()`, `state_attr()`, `is_state()`, `area_entities()`, `expand()`, фильтры, макросы из вашей папки `custom_templates`...):

- `entity` содержит сущность карточки, или дополнительной кнопки для поля дополнительной кнопки.
- `config.entity` содержит то же значение, для шаблонов, которые вы писали для card-mod.
- `user` содержит имя вошедшего пользователя.

Результаты разбираются Home Assistant точно так же, как в инструментах разработчика, поэтому `21.50` показывается как `21.5`. Добавьте `| string`, когда текст должен остаться как есть.

<details>

<summary>Шаблоны Home Assistant в ваших пользовательских стилях</summary>

<br>

Шаблон может содержать значение или оборачивать целые правила CSS:
...
JavaScript-шаблоны и шаблоны Home Assistant могут делить один блок. Держите каждый `${ }` вне блока `{% if %} ... {% endif %}`, каждая сторона отрисовывается своим движком, и блок, разрезанный на две части, отрисовать нельзя.

Вашему собственному тексту в строке состояния стили вовсе не нужны, `state_content` принимает шаблон как один из своих элементов:
...
Внутри JavaScript-шаблона `renderTemplate("{{ ... }}")` даёт вам отрисованный текст шаблона Home Assistant, для тех мест, куда шаблон сам дотянуться не может:
...
- `renderTemplate(template, entity)` возвращает текст, отрисованный Home Assistant для Jinja-шаблона, например `${card.querySelector('.bubble-state').innerText = renderTemplate("{{ states('sensor.humidity') }} %")}`. Второй аргумент задаёт то, что шаблон видит как `entity`, по умолчанию сущность вашей карточки.
...
Если вы хотите шаблонизировать состояние (`.bubble-state`) из ваших стилей, строка появляется на экране, как только шаблон что-то в неё записывает, независимо от того, что задано в `state_content`.

То же самое без каких-либо стилей, с шаблоном Home Assistant в `state_content`, который также даёт вам переведённое состояние:
```yaml
