...
Também funciona com qualquer entidade que expõe suas opções como uma lista de atributos: `hvac_modes`, `fan_modes`, `swing_modes`, `swing_horizontal_modes` e `preset_modes` em uma entidade de climatização, `available_modes` em um umidificador, `operation_list` em um aquecedor de água, `effect_list` em uma luz, `source_list` e `sound_mode_list` em um reprodutor de mídia.
...
Este cartão permite controlar suas entidades `climate`, `humidifier` e `water_heater`. Um umidificador, um desumidificador ou um higrostato genérico ganha os mesmos controles de mais e menos na sua umidade alvo, e um aquecedor de água na sua temperatura alvo.

> [!TIP]
> O menu de seleção de modo é um [sub-botão](#sub-botões) adicionado automaticamente ao criar o cartão. Você pode então modificá-lo ou removê-lo como quiser. Ele lê os `hvac_modes` de uma entidade de climatização, os `available_modes` de um umidificador e a `operation_list` de um aquecedor de água.
...
| Nome                     | Tipo    | Obrigatório                         | Opções suportadas                                  | Descrição                                                                                                     |
|--------------------------|---------|-------------------------------------|--------------------------------------------------|-----------------------------------------------------------------------------------------------------------------|
| `entity`                | string  | **Obrigatório**                        | Entidade de climatização, umidificador ou aquecedor de água | A entidade a ser controlada (ex.: `climate.living_room`, `humidifier.bedroom` ou `water_heater.boiler`).             |
...
| `state_color`           | boolean | Opcional                            | `true` ou `false` (padrão)                     | Aplica uma cor de fundo constante quando a entidade está ligada. Um aquecedor de água não tem equivalente de `hvac_action`, portanto essa é a única coisa que o colore. |
| `step` | number | Opcional | Qualquer número | O incremento do valor alvo, temperatura ou umidade. |
| `min_temp` | number | Opcional | Qualquer número | O valor alvo mínimo. Em um umidificador, é uma umidade, não uma temperatura. |
| `max_temp` | number | Opcional | Qualquer número | O valor alvo máximo. Em um umidificador, é uma umidade, não uma temperatura. |
| `button_action` | object | Opcional | `tap_action`, `double_tap_action` ou `hold_action`, veja [ações](#ações-de-toque-toque-duplo-e-toque-longo) | Permite alterar as ações padrão ao clicar no botão. |
| `tap_action` | object | Opcional | Veja [ações](#ações-de-toque-toque-duplo-e-toque-longo) | Define o tipo de ação ao clicar no ícone, se não for definido, será usado `more-info`. |
| `double_tap_action` | object | Opcional | Veja [ações](#ações-de-toque-toque-duplo-e-toque-longo) | Define o tipo de ação ao clicar duas vezes no ícone, se não for definido, será usado `none`. |
| `hold_action` | object | Opcional | Veja [ações](#ações-de-toque-toque-duplo-e-toque-longo) | Define o tipo de ação ao segurar o ícone, se não for definido, será usado `more-info`. |                              |
...
| `--bubble-state-humidifier-on-color` | `color` | Cor de sobreposição para um umidificador em funcionamento |
| `--bubble-state-humidifier-humidifier-on-color` | `color` | Cor de sobreposição para um umidificador em funcionamento, quando sua classe de dispositivo é `humidifier` |
| `--bubble-state-humidifier-dehumidifier-on-color` | `color` | Cor de sobreposição para um desumidificador em funcionamento, quando sua classe de dispositivo é `dehumidifier` |
| `--bubble-state-water_heater-<operation>-color` | `color` | Cor de sobreposição para um modo de operação do aquecedor de água, ex.: `--bubble-state-water_heater-eco-color` |
...
> [!NOTE]
> As condições são avaliadas no seu navegador, então as poucas que precisam do servidor do Home Assistant não podem ser exatas: o nascer e o pôr do sol são lidos da entidade `sun.sun` em vez de recalculados, e uma duração `for` é medida a partir da última mudança de estado, sem o histórico do recorder. A exceção é `template`, renderizado pelo servidor como qualquer outro [modelo do Home Assistant](#modelos).
>
...
O Bubble Card é compatível com dois tipos de modelos:

- **Modelos do Home Assistant (Jinja)**, os que você já escreve nas suas automações, no Mushroom ou no card-mod. Coloque `{{ ... }}` ou `{% ... %}` em um campo compatível e o Home Assistant o renderiza para você, ao vivo.
- **Modelos JavaScript**, `${ ... }` dentro dos seus [estilos personalizados](#estilo), para tudo o que precisa alcançar o próprio cartão.

### Modelos do Home Assistant (Jinja)

Os modelos são renderizados pelo seu servidor Home Assistant e se atualizam sozinhos quando o que eles leem muda. Eles funcionam nestes campos:

<details>

<summary><b>Campos compatíveis (com exemplos)</b></summary>

| Campo | Exemplo |
| --- | --- |
| `name`, em todos os cartões (cabeçalho do pop-up e separador incluídos) | `name: "{{ states('sensor.living_temp') }} °C"` |
| `icon`, em todos os cartões (`icon_open`, `icon_close`, `icon_up` e `icon_down` de uma cobertura também) | `icon: "{{ 'mdi:window-open' if is_state('binary_sensor.window', 'on') else 'mdi:window-closed' }}"` |
| `name` e `icon` de um [sub-botão](#sub-botões) | `name: "{{ 'Wet' if states(entity) \| float > 60 else 'Dry' }}"` |
| `state_content`, em um cartão ou um sub-botão, ao lado de `state` e dos nomes de atributos | `state_content: [state, "{{ states('sensor.humidity') }} %"]` |
| `1_name`, `1_icon`... de uma [pilha de botões horizontal](#pilha-de-botões-horizontal) | `1_name: "{{ user }}"` |
| `styles` de um cartão e o código de um [módulo](#módulos), misturados com modelos JavaScript | veja abaixo |
| [Condições](#condições), com `condition: template` | `value_template: "{{ is_state('sun.sun', 'below_horizon') }}"` |

</details>

> [!IMPORTANT]
> Sempre coloque um modelo entre aspas. Sem elas, `name: {{ states('x') }}` é lido pelo YAML como um mapeamento em vez de texto, e o cartão o recusa.

Três variáveis estão disponíveis além de tudo o que o Home Assistant oferece (`states()`, `state_attr()`, `is_state()`, `area_entities()`, `expand()`, filtros, as macros da sua pasta `custom_templates`...):

- `entity` é a entidade do cartão, ou do sub-botão para um campo de sub-botão.
- `config.entity` é o mesmo valor, para os modelos que você escreveu para o card-mod.
- `user` é o nome do usuário conectado.

Os resultados são interpretados pelo Home Assistant exatamente como nas ferramentas do desenvolvedor, então `21.50` aparece como `21.5`. Adicione `| string` quando o texto precisar ficar como está.

<details>

<summary>Modelos do Home Assistant nos seus estilos personalizados</summary>

<br>

Um modelo pode conter um valor ou envolver regras CSS inteiras:
...
Modelos JavaScript e modelos do Home Assistant podem compartilhar um bloco. Mantenha cada `${ }` fora de um bloco `{% if %} ... {% endif %}`, cada lado é renderizado por um motor diferente e um bloco cortado em dois não pode ser renderizado.

Seu próprio texto na linha de estado não precisa de nenhum estilo, `state_content` aceita um modelo como um de seus itens:
...
Dentro de um modelo JavaScript, `renderTemplate("{{ ... }}")` fornece o texto renderizado de um modelo do Home Assistant, para os lugares que um modelo não consegue alcançar sozinho:
...
Os erros são exibidos no editor, abaixo dos estilos personalizados, e no console do seu navegador.

</details>

### Modelos JavaScript

Usuários avançados podem adicionar modelos em JS diretamente nos seus [estilos personalizados](#estilo). Por exemplo, isso permite alterar dinamicamente um ícone, os textos ou as cores de um elemento, mostrar ou ocultar um elemento condicionalmente (como um sub-botão), ou quase qualquer coisa baseada em um estado, um atributo e muito mais.
...
- `renderTemplate(template, entity)` retorna o texto renderizado pelo Home Assistant para um modelo Jinja, por exemplo `${card.querySelector('.bubble-state').innerText = renderTemplate("{{ states('sensor.humidity') }} %")}`. O segundo argumento é o que o modelo vê como `entity`, a entidade do seu cartão por padrão.
...
Se você quiser aplicar um modelo ao estado (`.bubble-state`) a partir dos seus estilos, a linha aparece na tela assim que um modelo escreve nela, independentemente do que `state_content` diga.

O mesmo sem nenhum estilo, com um modelo do Home Assistant em `state_content`, que também fornece o estado traduzido:
```yaml
