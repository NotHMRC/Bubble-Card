...
**[`Instalación`](#instalación)**  **[`Configuración`](#configuración)**  **[`Pop-up`](#pop-up)**  **[`Pila de botones horizontal`](#pila-de-botones-horizontal)**  **[`Botón`](#botón)**  **[`Reproductor multimedia`](#reproductor-multimedia)**  **[`Persiana`](#persiana)**  **[`Selector`](#selector)**  **[`Clima`](#clima)**  **[`Calendario`](#calendario)**  **[`Separador`](#separador)**  **[`Columna vacía`](#columna-vacía)**  **[`Solo sub-botones`](#solo-sub-botones)**  **[`Sub-botones`](#sub-botones)**  **[`Sugerencias de entidad`](#sugerencias-de-entidad)**  **[`Diseños de tarjeta`](#diseños-de-tarjeta)**  **[`Condiciones`](#condiciones)**  **[`Acciones`](#acciones-de-toque-doble-toque-y-mantener-presionado)**  **[`Estilos`](#estilos)**  **[`Plantillas`](#plantillas)**  **[`Módulos`](#módulos)**  **[`Localización`](#localización)**  **[`Ayuda`](#ayuda)**  **[`Contribuir`](#contribuir)**  **[`Donar`](#donar)**
...
| `popup_style` | string | Opcional | `bubble` (predeterminado), `classic` o `home-assistant` | Define el estilo visual del pop-up |
...
> [!TIP]
> El menú de selección de modo es un [sub-botón](#sub-botones) que se agrega automáticamente al crear la tarjeta. Luego puedes modificarlo o eliminarlo como desees. Lee los `hvac_modes` de una entidad de clima, los `available_modes` de un humidificador y la `operation_list` de un calentador de agua.
...
| Nombre                     | Tipo    | Requisito                         | Opciones admitidas                                  | Descripción                                                                                                     |
|--------------------------|---------|-------------------------------------|--------------------------------------------------|-----------------------------------------------------------------------------------------------------------------|
| `entity`                | string  | **Requerido**                        | Entidad de clima, humidificador o calentador de agua | La entidad a controlar (por ejemplo, `climate.living_room`, `humidifier.bedroom` o `water_heater.boiler`).              |
...
| `state_content`         | string o list | Opcional | `state`, el nombre de un atributo, una plantilla | Lo que muestra la línea debajo del nombre, revisa las opciones del botón. La clave antigua `show_state` sigue funcionando. |
...
| `state_color`           | boolean | Opcional                            | `true` o `false` (predeterminado)                     | Aplica un color de fondo constante cuando la entidad está encendida. Un calentador de agua no tiene un equivalente de `hvac_action`, así que esto es lo único que le da color. |
...
| `--bubble-state-water_heater-<operation>-color` | `color` | Color de superposición para una operación de calentador de agua, por ejemplo `--bubble-state-water_heater-eco-color` |
...
| Campo | Ejemplo |
| --- | --- |
| `name`, en todas las tarjetas (encabezado del pop-up y separador incluidos) | `name: "{{ states('sensor.living_temp') }} °C"` |
...
- `user` es el nombre del usuario que inició sesión.

Home Assistant interpreta los resultados exactamente igual que en las herramientas para desarrolladores, así que `21.50` se muestra como `21.5`. Agrega `| string` cuando el texto deba quedarse tal cual.
...
Una plantilla puede contener un valor o envolver reglas CSS completas:
...
Dentro de una plantilla JavaScript, `renderTemplate("{{ ... }}")` te da el texto renderizado de una plantilla de Home Assistant, para los lugares a los que una plantilla no puede llegar por sí sola:
...
- `renderTemplate(template, entity)` devuelve el texto que renderiza Home Assistant para una plantilla Jinja, por ejemplo `${card.querySelector('.bubble-state').innerText = renderTemplate("{{ states('sensor.humidity') }} %")}`. El segundo argumento es lo que la plantilla ve como `entity`, la entidad de tu tarjeta de forma predeterminada.
...
Si quieres crear una plantilla para el estado (`.bubble-state`) desde tus estilos, la línea aparece en pantalla en cuanto una plantilla escribe en ella, sin importar lo que diga `state_content`.
