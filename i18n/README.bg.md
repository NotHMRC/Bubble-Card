...
**[`Инсталация`](#инсталация)**  **[`Конфигурация`](#конфигурация)**  **[`Pop-up`](#pop-up)**  **[`Хоризонтален стек от бутони`](#хоризонтален-стек-от-бутони)**  **[`Бутон`](#бутон)**  **[`Медиен плейър`](#медиен-плейър)**  **[`Щори`](#щори)**  **[`Избор`](#избор)**  **[`Климат`](#климат)**  **[`Календар`](#календар)**  **[`Разделител`](#разделител)**  **[`Празна колона`](#празна-колона)**  **[`Само подбутони`](#само-подбутони)**  **[`Подбутони`](#подбутони)**  **[`Предложения за обекти`](#предложения-за-обекти)**  **[`Подредби на картата`](#подредби-на-картата)**  **[`Условия`](#условия)**  **[`Действия`](#действия-при-докосване-двойно-докосване-и-задържане)**  **[`Оформление`](#оформление)**  **[`Шаблони`](#шаблони)**  **[`Модули (Modules)`](#модули-modules)**  **[`Локализация`](#локализация)**  **[`Помощ`](#помощ)**  **[`Принос`](#принос)**  **[`Дарения`](#дарения)**
...
| `popup_style` | string | По избор | `bubble` (по подразбиране), `classic` или `home-assistant` | Определя визуалния стил на изскачащия прозорец |
...
| `state_content` | string или list | Незадължително | `state`, `last-changed`, `last-updated`, `last-triggered`, име на атрибут като `brightness` или `forecast[0].temperature`, или [шаблон](#шаблони) | Какво показва редът под името, в тази последователност. Без него бутон с `button_type: state` показва това, което Home Assistant показва за обекта (състоянието му, плюс текущата температура на климатик, позицията на щори, яркостта на осветление). Старите ключове `show_state`, `show_attribute`, `attribute`, `show_last_changed` и `show_last_updated` продължават да работят и се пренаписват като `state_content`, когато отворите редактора. |
...
Работи също и с всеки обект, който предоставя опциите си като атрибут списък: `hvac_modes`, `fan_modes`, `swing_modes`, `swing_horizontal_modes` и `preset_modes` при климатичен обект, `available_modes` при овлажнител, `operation_list` при бойлер, `effect_list` при осветителен обект, `source_list` и `sound_mode_list` при медиен плейър.
...
Тази карта ви позволява да управлявате вашите `climate`, `humidifier` и `water_heater` обекти. Овлажнител, изсушител или общ хигростат получава същите бутони плюс и минус за целевата си влажност, а бойлерът за целевата си температура.

> [!TIP]
> Менюто за избор на режим е [подбутон](#подбутони), който се добавя автоматично при създаването на картата. След това можете да го променяте или премахвате, както желаете. То чете `hvac_modes` на климатичен обект, `available_modes` на овлажнител и `operation_list` на бойлер.
...
| Име                     | Тип    | Изискване                         | Поддържани опции                                  | Описание                                                                                                     |
|--------------------------|---------|-------------------------------------|--------------------------------------------------|-----------------------------------------------------------------------------------------------------------------|
| `entity`                | string  | **Задължително**                        | Климатичен обект, овлажнител или бойлер            | Обектът за управление (напр. `climate.living_room`, `humidifier.bedroom` или `water_heater.boiler`).            |
...
| `force_icon`            | boolean | Незадължително                            | `true` или `false` (по подразбиране)                     | Дава приоритет на иконата пред `entity-picture`.                                                           |
| `state_content`         | string или list | Незадължително | `state`, име на атрибут, шаблон | Какво показва редът под името, вижте опциите на бутона. Старият ключ `show_state` продължава да работи. |
...
| `state_color`           | boolean | Незадължително                            | `true` или `false` (по подразбиране)                     | Прилага постоянен цвят на фона, когато обектът е включен. Бойлерът няма еквивалент на `hvac_action`, така че това е единственото, което го оцветява. |
| `step` | number | Незадължително | Всяко число | Стъпката на целевата стойност, температура или влажност. |
| `min_temp` | number | Незадължително | Всяко число | Минималната целева стойност. При овлажнител това е влажност, а не температура. |
| `max_temp` | number | Незадължително | Всяко число | Максималната целева стойност. При овлажнител това е влажност, а не температура. |
| `button_action` | object | Незадължително | `tap_action`, `double_tap_action` или `hold_action`, вижте [действията](#действия-при-докосване-двойно-докосване-и-задържане) | Позволява да промените действията по подразбиране при докосване на бутона. |
| `tap_action` | object | Незадължително | Вижте [действията](#действия-при-докосване-двойно-докосване-и-задържане) | Определя типа действие при докосване на иконата, ако не е зададено, ще се използва `more-info`. |
| `double_tap_action` | object | Незадължително | Вижте [действията](#действия-при-докосване-двойно-докосване-и-задържане) | Определя типа действие при двойно докосване на иконата, ако не е зададено, ще се използва `none`. |
| `hold_action` | object | Незадължително | Вижте [действията](#действия-при-докосване-двойно-докосване-и-задържане) | Определя типа действие при задържане на иконата, ако не е зададено, ще се използва `more-info`. |                              |
...
| `--bubble-state-humidifier-on-color` | `color` | Цвят на наслагването за работещ овлажнител |
| `--bubble-state-humidifier-humidifier-on-color` | `color` | Цвят на наслагването за работещ овлажнител, когато класът на устройството му е `humidifier` |
| `--bubble-state-humidifier-dehumidifier-on-color` | `color` | Цвят на наслагването за работещ изсушител, когато класът на устройството му е `dehumidifier` |
| `--bubble-state-water_heater-<operation>-color` | `color` | Цвят на наслагването за режим на работа на бойлер, например `--bubble-state-water_heater-eco-color` |
...
| `state_content` | string или list | Незадължителна | `state`, `last-changed`, `last-updated`, `last-triggered`, име на атрибут като `brightness` или `forecast[0].temperature`, или [шаблон](#шаблони) | Какво показва редът под името, в тази последователност. Без него бутон с `button_type: state` показва това, което Home Assistant показва за обекта (състоянието му, плюс текущата температура на климатик, позицията на щори, яркостта на осветление). Старите ключове `show_state`, `show_attribute`, `attribute`, `show_last_changed` и `show_last_updated` продължават да работят и се пренаписват като `state_content`, когато отворите редактора. |
...
Bubble Card поддържа два вида шаблони:

- **Шаблони на Home Assistant (Jinja)**, тези, които вече пишете в автоматизациите си, в Mushroom или в card-mod. Поставете `{{ ... }}` или `{% ... %}` в поддържано поле и Home Assistant го изобразява вместо вас, на живо.
- **JavaScript шаблони**, `${ ... }` във вашите [персонализирани стилове](#оформление), за всичко, което трябва да достигне до самата карта.

### Шаблони на Home Assistant (Jinja)

Шаблоните се изобразяват от вашия сървър на Home Assistant и се обновяват сами, когато това, което четат, се промени. Те работят в следните полета:

<details>

<summary><b>Поддържани полета (с примери)</b></summary>

| Поле | Пример |
| --- | --- |
| `name`, на всяка карта (включително заглавната част на изскачащия прозорец и разделителя) | `name: "{{ states('sensor.living_temp') }} °C"` |
| `icon`, на всяка карта (също и `icon_open`, `icon_close`, `icon_up` и `icon_down` на щори) | `icon: "{{ 'mdi:window-open' if is_state('binary_sensor.window', 'on') else 'mdi:window-closed' }}"` |
| `name` и `icon` на [подбутон](#подбутони) | `name: "{{ 'Wet' if states(entity) \| float > 60 else 'Dry' }}"` |
| `state_content`, на карта или подбутон, редом със `state` и имената на атрибути | `state_content: [state, "{{ states('sensor.humidity') }} %"]` |
| `1_name`, `1_icon`... на [хоризонтален стек от бутони](#хоризонтален-стек-от-бутони) | `1_name: "{{ user }}"` |
| `styles` на карта и кодът на [модул](#модули-modules), смесени с JavaScript шаблони | вижте по-долу |
| [Условия](#условия), с `condition: template` | `value_template: "{{ is_state('sun.sun', 'below_horizon') }}"` |

</details>

> [!IMPORTANT]
> Винаги поставяйте шаблона в кавички. Без тях `name: {{ states('x') }}` се чете от YAML като mapping, а не като текст, и картата го отхвърля.

Налични са три променливи в допълнение към всичко, което Home Assistant предлага (`states()`, `state_attr()`, `is_state()`, `area_entities()`, `expand()`, филтри, макросите от папката ви `custom_templates`...):

- `entity` е обектът на картата, или на подбутона при поле на подбутон.
- `config.entity` е същата стойност, за шаблоните, които сте писали за card-mod.
- `user` е името на влезлия потребител.

Резултатите се обработват от Home Assistant точно както в инструментите за разработчици, така че `21.50` се показва като `21.5`. Добавете `| string`, когато текстът трябва да остане такъв, какъвто е.

<details>

<summary>Шаблони на Home Assistant във вашите персонализирани стилове</summary>

<br>

Шаблонът може да съдържа стойност или да обгръща цели CSS правила:
...
JavaScript шаблоните и шаблоните на Home Assistant могат да споделят един блок. Дръжте всеки `${ }` извън блок `{% if %} ... {% endif %}`, всяка страна се изобразява от различен механизъм и блок, разрязан на две, не може да бъде изобразен.

Вашият собствен текст на реда за състоянието не се нуждае от никакви стилове, `state_content` приема шаблон като един от елементите си:
...
В JavaScript шаблон `renderTemplate("{{ ... }}")` ви дава изобразения текст на шаблон на Home Assistant, за местата, до които шаблонът не може да стигне сам:
...
- `renderTemplate(template, entity)` връща текста, изобразен от Home Assistant за Jinja шаблон, например `${card.querySelector('.bubble-state').innerText = renderTemplate("{{ states('sensor.humidity') }} %")}`. Вторият аргумент е това, което шаблонът вижда като `entity`, по подразбиране обектът на картата ви.
...
Ако искате да използвате шаблон за състоянието (`.bubble-state`) от вашите стилове, редът се появява на екрана веднага щом шаблон запише в него, каквото и да казва `state_content`.

Същото без никакви стилове, с шаблон на Home Assistant в `state_content`, което ви дава и преведеното състояние:
```yaml
