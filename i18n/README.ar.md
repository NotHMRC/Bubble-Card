...
**[`التثبيت`](#التثبيت)**  **[`الإعداد`](#الإعداد)**  **[`Pop-up`](#pop-up)**  **[`تكديس الأزرار الأفقي`](#تكديس-الأزرار-الأفقي)**  **[`الزر`](#الزر)**  **[`مشغل الوسائط`](#مشغل-الوسائط)**  **[`الستارة`](#الستارة)**  **[`الاختيار`](#الاختيار)**  **[`التكييف`](#التكييف)**  **[`التقويم`](#التقويم)**  **[`الفاصل`](#الفاصل)**  **[`العمود الفارغ`](#العمود-الفارغ)**  **[`الأزرار الفرعية فقط`](#الأزرار-الفرعية-فقط)**  **[`الأزرار الفرعية`](#الأزرار-الفرعية)**  **[`اقتراحات الكيانات`](#اقتراحات-الكيانات)**  **[`تخطيطات البطاقة`](#تخطيطات-البطاقة)**  **[`الشروط`](#الشروط)**  **[`الإجراءات`](#إجراءات-النقر-والنقر-المزدوج-والضغط-المطول)**  **[`التنسيق`](#التنسيق)**  **[`القوالب`](#القوالب)**  **[`Modules`](#modules)**  **[`الترجمة`](#الترجمة)**  **[`المساعدة`](#المساعدة)**  **[`المساهمة`](#المساهمة)**  **[`التبرع`](#التبرع)**
...
تعمل أيضًا مع أي كيان يعرض خياراته على شكل قائمة سمات: `hvac_modes` و`fan_modes` و`swing_modes` و`swing_horizontal_modes` و`preset_modes` في كيان التكييف، و`available_modes` في المرطب، و`operation_list` في سخان المياه، و`effect_list` في الإضاءة، و`source_list` و`sound_mode_list` في مشغل الوسائط.
...
تتيح لك هذه البطاقة التحكم في كيانات `climate` و`humidifier` و`water_heater` الخاصة بك. يحصل المرطب أو مزيل الرطوبة أو منظم الرطوبة العام على أزرار الزائد والناقص نفسها للرطوبة المستهدفة، ويحصل عليها سخان المياه لدرجة الحرارة المستهدفة.

> [!TIP]
> قائمة اختيار الوضع هي [زر فرعي](#الأزرار-الفرعية) يُضاف تلقائيًا عند إنشاء البطاقة. يمكنك بعد ذلك تعديله أو إزالته كما تشاء. وهي تقرأ `hvac_modes` من كيان التكييف، و`available_modes` من المرطب، و`operation_list` من سخان المياه.
...
| الاسم                     | النوع    | الإلزامية                         | الخيارات المدعومة                                  | الوصف                                                                                                     |
|--------------------------|---------|-------------------------------------|--------------------------------------------------|-----------------------------------------------------------------------------------------------------------------|
| `entity`                | string  | **إلزامي**                        | كيان تكييف أو مرطب أو سخان مياه              | الكيان المراد التحكم فيه (مثل `climate.living_room` أو `humidifier.bedroom` أو `water_heater.boiler`).           |
...
| `state_color`           | boolean | اختياري                            | `true` أو `false` (افتراضيًا)                     | تطبيق لون خلفية ثابت عندما يكون الكيان قيد التشغيل. لا يملك سخان المياه ما يعادل `hvac_action`، لذا فهذا هو الشيء الوحيد الذي يلوّنه. |
| `step` | number | اختياري | أي رقم | مقدار خطوة القيمة المستهدفة، درجة حرارة كانت أو رطوبة. |
| `min_temp` | number | اختياري | أي رقم | القيمة المستهدفة الدنيا. في المرطب تكون رطوبة وليست درجة حرارة. |
| `max_temp` | number | اختياري | أي رقم | القيمة المستهدفة القصوى. في المرطب تكون رطوبة وليست درجة حرارة. |
| `button_action` | object | اختياري | `tap_action` أو `double_tap_action` أو `hold_action`، انظر [الإجراءات](#إجراءات-النقر-والنقر-المزدوج-والضغط-المطول) | يتيح تغيير الإجراءات الافتراضية عند النقر على الزر. |
| `tap_action` | object | اختياري | انظر [الإجراءات](#إجراءات-النقر-والنقر-المزدوج-والضغط-المطول) | تحديد نوع الإجراء عند النقر على الأيقونة، إذا لم يُحدد فسيُستخدم `more-info`. |
| `double_tap_action` | object | اختياري | انظر [الإجراءات](#إجراءات-النقر-والنقر-المزدوج-والضغط-المطول) | تحديد نوع الإجراء عند النقر المزدوج على الأيقونة، إذا لم يُحدد فسيُستخدم `none`. |
| `hold_action` | object | اختياري | انظر [الإجراءات](#إجراءات-النقر-والنقر-المزدوج-والضغط-المطول) | تحديد نوع الإجراء عند الضغط المطول على الأيقونة، إذا لم يُحدد فسيُستخدم `more-info`. |                              |
...
| `--bubble-state-humidifier-on-color` | `color` | لون التراكب لمرطب قيد التشغيل |
| `--bubble-state-humidifier-humidifier-on-color` | `color` | لون التراكب لمرطب قيد التشغيل، عندما تكون فئة جهازه `humidifier` |
| `--bubble-state-humidifier-dehumidifier-on-color` | `color` | لون التراكب لمزيل رطوبة قيد التشغيل، عندما تكون فئة جهازه `dehumidifier` |
| `--bubble-state-water_heater-<operation>-color` | `color` | لون التراكب لوضع تشغيل سخان المياه، مثل `--bubble-state-water_heater-eco-color` |
