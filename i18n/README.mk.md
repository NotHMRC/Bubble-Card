...
**[`Инсталација`](#инсталација)**  **[`Конфигурација`](#конфигурација)**  **[`Скокачки прозорец`](#скокачки-прозорец)**  **[`Хоризонтален стек од копчиња`](#хоризонтален-стек-од-копчиња)**  **[`Копче`](#копче)**  **[`Медиа плеер`](#медиа-плеер)**  **[`Ролетни`](#ролетни)**  **[`Select`](#select)**  **[`Клима`](#клима)**  **[`Календар`](#календар)**  **[`Разделник`](#разделник)**  **[`Празна колона`](#празна-колона)**  **[`Само подкопчиња`](#само-подкопчиња)**  **[`Подкопчиња`](#подкопчиња)**  **[`Предлози за ентитети`](#предлози-за-ентитети)**  **[`Распореди на картичките`](#распореди-на-картичките)**  **[`Услови`](#услови)**  **[`Дејства`](#дејства-на-допир-двоен-допир-и-задржување)**  **[`Стилови`](#стилови)**  **[`Шаблони`](#шаблони)**  **[`Модули`](#модули)**  **[`Локализација`](#локализација)**  **[`Помош`](#помош)**  **[`Придонесување`](#придонесување)**  **[`Донирајте`](#донирајте)**
...
| `popup_style` | string | Опционално | `bubble` (стандардно), `classic` или `home-assistant` | Го дефинира визуелниот стил на скокачкиот прозорец |
...
Исто така работи со кој било ентитет што ги изложува своите опции како список на атрибути: `hvac_modes`, `fan_modes`, `swing_modes`, `swing_horizontal_modes` и `preset_modes` кај клима ентитет, `available_modes` кај овлажнувач, `operation_list` кај грејач на вода, `effect_list` кај светло, `source_list` и `sound_mode_list` кај медиа плеер.
...
Оваа картичка ви овозможува да ги контролирате вашите ентитети `climate`, `humidifier` и `water_heater`. Овлажнувач, одвлажнувач или општ хигростат ги добива истите контроли плус и минус за својата целна влажност, а грејач на вода за својата целна температура.

> [!TIP]
> Менито за избор на режим е [подкопче](#подкопчиња) кое се додава автоматски при креирање на картичката. Потоа можете да го измените или отстраните како што сакате. Го чита `hvac_modes` на клима ентитет, `available_modes` на овлажнувач и `operation_list` на грејач на вода.
...
| Име                     | Тип    | Барање                         | Поддржани опции                                  | Опис                                                                                                     |
|--------------------------|---------|-------------------------------------|--------------------------------------------------|-----------------------------------------------------------------------------------------------------------------|
| `entity`                | string  | **Задолжително**                        | Ентитет за клима, овлажнувач или грејач на вода   | Ентитетот за контрола (на пр. `climate.living_room`, `humidifier.bedroom` или `water_heater.boiler`).            |
...
| `state_color`           | boolean | Опционално                            | `true` или `false` (стандардно)                     | Применува постојана боја на позадината кога ентитетот е вклучен. Грејачот на вода нема еквивалент на `hvac_action`, па ова е единственото нешто што го обојува. |
| `step` | number | Опционално | Кој било број | Чекорот на целната вредност, температура или влажност. |
| `min_temp` | number | Опционално | Кој било број | Минималната целна вредност. Кај овлажнувач ова е влажност, а не температура. |
| `max_temp` | number | Опционално | Кој било број | Максималната целна вредност. Кај овлажнувач ова е влажност, а не температура. |
| `button_action` | object | Опционално | `tap_action`, `double_tap_action` или `hold_action`, види [дејства](#дејства-на-допир-двоен-допир-и-задржување) | Овозможува промена на стандардните дејства при клик на копчето. |
| `tap_action` | object | Опционално | Види [дејства](#дејства-на-допир-двоен-допир-и-задржување) | Дефинирај го типот на дејство при клик на иконата, ако не е дефинирано, ќе се користи `more-info`. |
| `double_tap_action` | object | Опционално | Види [дејства](#дејства-на-допир-двоен-допир-и-задржување) | Дефинирај го типот на дејство при двоен клик на иконата, ако не е дефинирано, ќе се користи `none`. |
| `hold_action` | object | Опционално | Види [дејства](#дејства-на-допир-двоен-допир-и-задржување) | Дефинирај го типот на дејство при задржување на иконата, ако не е дефинирано, ќе се користи `more-info`. |                              |
...
| `--bubble-state-humidifier-on-color` | `color` | Боја на преклопување за овлажнувач што работи |
| `--bubble-state-humidifier-humidifier-on-color` | `color` | Боја на преклопување за овлажнувач што работи, кога неговата класа на уред е `humidifier` |
| `--bubble-state-humidifier-dehumidifier-on-color` | `color` | Боја на преклопување за одвлажнувач што работи, кога неговата класа на уред е `dehumidifier` |
| `--bubble-state-water_heater-<operation>-color` | `color` | Боја на преклопување за режим на работа на грејач на вода, на пр. `--bubble-state-water_heater-eco-color` |
...
> [!NOTE]
> Условите се проценуваат во вашиот прелистувач, па оние неколкуте на кои им треба серверот на Home Assistant не можат да бидат точни: изгрејсонцето и зајдисонцето се читаат од ентитетот `sun.sun` наместо повторно да се пресметуваат, а траењето `for` се мери од последната промена на состојбата, без историјата на recorder. Исклучок е `template`, што серверот го рендерира како кој било друг [Home Assistant шаблон](#шаблони).
>
...
Bubble Card поддржува два вида шаблони:

- **Home Assistant шаблони (Jinja)**, оние што веќе ги пишувате во вашите автоматизации, во Mushroom или во card-mod. Ставете `{{ ... }}` или `{% ... %}` во поддржано поле и Home Assistant го рендерира за вас, во живо.
- **JavaScript шаблони**, `${ ... }` во вашите [сопствени стилови](#стилови), за сè што треба да допре до самата картичка.

### Home Assistant шаблони (Jinja)

Шаблоните се рендерираат од вашиот сервер на Home Assistant и се ажурираат сами кога ќе се промени тоа што го читаат. Работат во овие полиња:

<details>

<summary><b>Поддржани полиња (со примери)</b></summary>

| Поле | Пример |
| --- | --- |
| `name`, на секоја картичка (вклучувајќи го заглавието на скокачкиот прозорец и разделникот) | `name: "{{ states('sensor.living_temp') }} °C"` |
| `icon`, на секоја картичка (и `icon_open`, `icon_close`, `icon_up` и `icon_down` на ролетна) | `icon: "{{ 'mdi:window-open' if is_state('binary_sensor.window', 'on') else 'mdi:window-closed' }}"` |
| `name` и `icon` на [подкопче](#подкопчиња) | `name: "{{ 'Wet' if states(entity) \| float > 60 else 'Dry' }}"` |
| `state_content`, на картичка или подкопче, покрај `state` и имињата на атрибути | `state_content: [state, "{{ states('sensor.humidity') }} %"]` |
| `1_name`, `1_icon`... на [хоризонтален стек од копчиња](#хоризонтален-стек-од-копчиња) | `1_name: "{{ user }}"` |
| `styles` на картичка и кодот на [модул](#модули), измешани со JavaScript шаблони | видете подолу |
| [Услови](#услови), со `condition: template` | `value_template: "{{ is_state('sun.sun', 'below_horizon') }}"` |

</details>

> [!IMPORTANT]
> Секогаш ставајте шаблон меѓу наводници. Без нив, YAML го чита `name: {{ states('x') }}` како мапирање наместо како текст, и картичката го одбива.

Три променливи се достапни покрај сè што нуди Home Assistant (`states()`, `state_attr()`, `is_state()`, `area_entities()`, `expand()`, филтри, макроата од вашата папка `custom_templates`...):

- `entity` е ентитетот на картичката, или на подкопчето за поле на подкопче.
- `config.entity` е истата вредност, за шаблоните што ги напишавте за card-mod.
- `user` е името на пријавениот корисник.

Резултатите се парсираат од Home Assistant точно како во алатките за развивачи, па `21.50` се прикажува како `21.5`. Додајте `| string` кога текстот мора да остане како што е.

<details>

<summary>Home Assistant шаблони во вашите сопствени стилови</summary>

<br>

Шаблон може да содржи вредност или да опфати цели CSS правила:
...
JavaScript шаблоните и Home Assistant шаблоните можат да делат ист блок. Држете го секој `${ }` надвор од блок `{% if %} ... {% endif %}`, секоја страна се рендерира од различен мотор и блок пресечен на два дела не може да се рендерира.

Вашиот сопствен текст во линијата на состојбата не бара никакви стилови, `state_content` прима шаблон како една од своите ставки:
...
Во JavaScript шаблон, `renderTemplate("{{ ... }}")` ви го дава рендерираниот текст на Home Assistant шаблон, за местата до кои шаблон не може да допре сам:
...
Грешките се прикажуваат во едиторот, под сопствените стилови, и во конзолата на вашиот прелистувач.

</details>

### JavaScript шаблони

Напредните корисници можат да додадат шаблони во JS директно во нивните [сопствени стилови](#стилови). На пример, ова овозможува динамично менување на икона, текстовите или боите на елемент, прикажување или скривање на елемент условно (како подкопче), или речиси сè друго врз основа на состојба, атрибут и уште многу.
...
- `renderTemplate(template, entity)` го враќа текстот што Home Assistant го рендерирал за Jinja шаблон, на пример `${card.querySelector('.bubble-state').innerText = renderTemplate("{{ states('sensor.humidity') }} %")}`. Вториот аргумент е тоа што шаблонот го гледа како `entity`, стандардно ентитетот на вашата картичка.
...
Ако сакате да ја шаблонирате состојбата (`.bubble-state`) од вашите стилови, редот се појавува на екранот штом некој шаблон ќе запише нешто во него, без оглед на тоа што вели `state_content`.

Истото без никакви стилови, со Home Assistant шаблон во `state_content`, што ви ја дава и преведената состојба:
```yaml
