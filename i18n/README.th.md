...
การ์ดนี้ยังใช้ได้กับเอนทิตีใด ๆ ที่แสดงตัวเลือกของตัวเองเป็นรายการแอตทริบิวต์ ได้แก่ `hvac_modes`, `fan_modes`, `swing_modes`, `swing_horizontal_modes` และ `preset_modes` ของเครื่องปรับอากาศ, `available_modes` ของเครื่องเพิ่มความชื้น, `operation_list` ของเครื่องทำน้ำอุ่น, `effect_list` ของไฟ, `source_list` และ `sound_mode_list` ของเครื่องเล่นสื่อ
...
การ์ดนี้ให้คุณควบคุมเอนทิตี `climate`, `humidifier` และ `water_heater` ของคุณได้ เครื่องเพิ่มความชื้น เครื่องลดความชื้น หรือไฮโกรสตัททั่วไปจะได้ปุ่มบวกและลบแบบเดียวกันสำหรับความชื้นเป้าหมาย ส่วนเครื่องทำน้ำอุ่นจะได้สำหรับอุณหภูมิเป้าหมาย

> [!TIP]
> เมนูเลือกโหมด (mode selection) เป็น [ปุ่มย่อย](#ปุ่มย่อย) ที่ถูกเพิ่มโดยอัตโนมัติเมื่อสร้างการ์ด คุณสามารถแก้ไขหรือลบมันได้ตามต้องการ โดยจะอ่าน `hvac_modes` ของเอนทิตีปรับอากาศ `available_modes` ของเครื่องเพิ่มความชื้น และ `operation_list` ของเครื่องทำน้ำอุ่น
...
| ชื่อ                     | ประเภท    | ข้อกำหนด                         | ตัวเลือกที่รองรับ                                  | คำอธิบาย                                                                                                     |
|--------------------------|---------|-------------------------------------|--------------------------------------------------|-----------------------------------------------------------------------------------------------------------------|
| `entity`                | string  | **จำเป็น**                        | เอนทิตีปรับอากาศ เครื่องเพิ่มความชื้น หรือเครื่องทำน้ำอุ่น | เอนทิตีที่จะควบคุม (เช่น `climate.living_room`, `humidifier.bedroom` หรือ `water_heater.boiler`)           |
...
| `force_icon`            | boolean | ไม่บังคับ                            | `true` หรือ `false` (ค่าเริ่มต้น)                     | ให้ความสำคัญกับไอคอนแทนที่จะเป็น `entity-picture`                                                           |
| `state_content`         | string หรือ list | ไม่บังคับ | `state`, ชื่อแอตทริบิวต์, เทมเพลต | สิ่งที่บรรทัดใต้ชื่อจะแสดง ดูตัวเลือกของปุ่ม คีย์เก่า `show_state` ยังใช้ได้อยู่ |
...
| `state_color`           | boolean | ไม่บังคับ                            | `true` หรือ `false` (ค่าเริ่มต้น)                     | ใช้สีพื้นหลังคงที่เมื่อเอนทิตีเปิดอยู่ (ON) เครื่องทำน้ำอุ่นไม่มีสิ่งที่เทียบเท่ากับ `hvac_action` ดังนั้นนี่จึงเป็นสิ่งเดียวที่ให้สีกับมัน |
| `step` | number | ไม่บังคับ | ตัวเลขใด ๆ | ค่าขั้นของค่าเป้าหมาย ไม่ว่าจะเป็นอุณหภูมิหรือความชื้น |
| `min_temp` | number | ไม่บังคับ | ตัวเลขใด ๆ | ค่าเป้าหมายต่ำสุด สำหรับเครื่องเพิ่มความชื้นค่านี้คือความชื้น ไม่ใช่อุณหภูมิ |
| `max_temp` | number | ไม่บังคับ | ตัวเลขใด ๆ | ค่าเป้าหมายสูงสุด สำหรับเครื่องเพิ่มความชื้นค่านี้คือความชื้น ไม่ใช่อุณหภูมิ |
| `button_action` | object | ไม่บังคับ | `tap_action`, `double_tap_action` หรือ `hold_action` ดู [การกระทำ](#การกระทำเมื่อแตะ-แตะสองครั้ง-และกดค้าง) | ให้เปลี่ยนการกระทำเริ่มต้นเมื่อคลิกปุ่ม |
| `tap_action` | object | ไม่บังคับ | ดู [การกระทำ](#การกระทำเมื่อแตะ-แตะสองครั้ง-และกดค้าง) | กำหนดประเภทการกระทำเมื่อคลิกไอคอน ถ้าไม่กำหนดจะใช้ `more-info` |
| `double_tap_action` | object | ไม่บังคับ | ดู [การกระทำ](#การกระทำเมื่อแตะ-แตะสองครั้ง-และกดค้าง) | กำหนดประเภทการกระทำเมื่อดับเบิลคลิกไอคอน ถ้าไม่กำหนดจะใช้ `none` |
| `hold_action` | object | ไม่บังคับ | ดู [การกระทำ](#การกระทำเมื่อแตะ-แตะสองครั้ง-และกดค้าง) | กำหนดประเภทการกระทำเมื่อกดค้างไอคอน ถ้าไม่กำหนดจะใช้ `more-info` |                              |
...
| `--bubble-state-humidifier-on-color` | `color` | สีทับซ้อนสำหรับเครื่องเพิ่มความชื้นที่กำลังทำงาน |
| `--bubble-state-humidifier-humidifier-on-color` | `color` | สีทับซ้อนสำหรับเครื่องเพิ่มความชื้นที่กำลังทำงาน เมื่อ device class ของมันคือ `humidifier` |
| `--bubble-state-humidifier-dehumidifier-on-color` | `color` | สีทับซ้อนสำหรับเครื่องลดความชื้นที่กำลังทำงาน เมื่อ device class ของมันคือ `dehumidifier` |
| `--bubble-state-water_heater-<operation>-color` | `color` | สีทับซ้อนสำหรับโหมดการทำงานของเครื่องทำน้ำอุ่น เช่น `--bubble-state-water_heater-eco-color` |
